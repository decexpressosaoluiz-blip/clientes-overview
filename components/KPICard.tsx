...
export const KPICard: React.FC<KPICardProps> = memo(({ 
  ...
  return (
    <div 
      onClick={onClick}
      className={`
        relative p-6 rounded-3xl transition-all duration-300 flex flex-col justify-between h-full group select-none bg-white border
        ${isActive 
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className={`
          p-3 rounded-2xl transition-all duration-300 shadow-sm group-hover:scale-110 group-hover:rotate-3
          ${isRed 
      ...
      <div className="flex-1 flex flex-col justify-end">
        <h3 className="text-sle-neutral-400 text-[10px] font-extrabold uppercase tracking-widest mb-1">{title}</h3>
        <div className="text-[26px] font-extrabold text-sle-neutral-900 tracking-tight leading-none">
          {value}
        </div>
        {subValue && (
          <div className={`text-[11px] font-bold mt-3 inline-flex items-center px-2.5 py-1 rounded-lg transition-colors w-fit ${
            isRed 
      ...
      {onClick && (
        <div className="mt-4 pt-4 border-t border-sle-neutral-50 flex items-center text-[10px] font-bold text-sle-neutral-400 group-hover:text-indigo-600 transition-colors uppercase tracking-widest">
