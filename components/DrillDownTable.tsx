...
export const DrillDownTable: React.FC<DrillDownTableProps> = ({ title, clients, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-6 animate-in fade-in duration-200">
      ...
      <div className="relative bg-white rounded-none sm:rounded-[2rem] shadow-2xl w-full max-w-6xl h-full sm:h-[85vh] flex flex-col animate-in slide-in-from-bottom-5 sm:zoom-in-95 duration-300 border border-white/20 ring-1 ring-black/5 overflow-hidden select-none">
        
        <div className="px-6 sm:px-8 py-5 sm:py-6 border-b border-sle-neutral-100 flex justify-between items-center bg-white sticky top-0 z-20">
          <div>
            <h2 className="text-xl sm:text-2xl font-extrabold text-sle-neutral-900 tracking-tight">{title}</h2>
            <div className="flex items-center mt-1 space-x-2">
                <span className="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-lg text-[10px] sm:text-[11px] font-extrabold uppercase tracking-wide border border-indigo-100">
        ...
        <div className="flex-1 overflow-auto custom-scrollbar bg-white">
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-10 bg-white/95 backdrop-blur border-b border-sle-neutral-100 shadow-sm">
              <tr>
                <th className="px-6 sm:px-8 py-4 sm:py-5 text-[10px] font-extrabold text-sle-neutral-400 uppercase tracking-widest whitespace-nowrap">Cliente</th>
                <th className="px-6 sm:px-8 py-4 sm:py-5 text-[10px] font-extrabold text-sle-neutral-400 uppercase tracking-widest text-right whitespace-nowrap">Faturamento</th>
                <th className="px-6 sm:px-8 py-4 sm:py-5 text-[10px] font-extrabold text-sle-neutral-400 uppercase tracking-widest text-center whitespace-nowrap">Envios</th>
                <th className="px-6 sm:px-8 py-4 sm:py-5 text-[10px] font-extrabold text-sle-neutral-400 uppercase tracking-widest whitespace-nowrap">Última Atividade</th>
                <th className="px-6 sm:px-8 py-4 sm:py-5 text-[10px] font-extrabold text-sle-neutral-400 uppercase tracking-widest text-center whitespace-nowrap">Status</th>
              </tr>
            </thead>
                ...
                >
                  <td className="px-6 sm:px-8 py-4 sm:py-5">
                    <div className="flex items-center min-w-[200px]">
                        <span className={`
                            mr-4 w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shadow-sm shrink-0
                            ${idx < 3 ? 'bg-amber-100 text-amber-700' : 'bg-sle-neutral-100 text-sle-neutral-400'}
                        `}>
                            {idx + 1}
                        </span>
                        <div className="min-w-0">
                            <p className="text-sm font-bold text-sle-neutral-900 group-hover:text-indigo-600 transition-colors truncate max-w-[150px] sm:max-w-xs">{client.name}</p>
                  ...
                  <td className="px-6 sm:px-8 py-4 sm:py-5 whitespace-nowrap">
                   ...
                   <td className="px-6 sm:px-8 py-4 sm:py-5 text-center whitespace-nowrap">
        ...
        <div className="p-4 bg-white border-t border-sle-neutral-100 flex justify-end sticky bottom-0">
            <button onClick={onClose} className="w-full sm:w-auto px-6 py-3 bg-sle-neutral-900 text-white rounded-xl hover:bg-sle-neutral-800 transition-all text-xs font-bold shadow-lg hover:shadow-xl active:scale-95 cursor-pointer uppercase tracking-wider">
                Fechar Visualização
            </button>
        </div>
      </div>
